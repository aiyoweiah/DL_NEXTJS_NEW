...
export const home = {
  meta: {
    title:
      'DODO 都学书院 — 导师亲授的英文读写，从 5 岁到高中 | Think Once. In Both Languages.',
    description:
      '由导师（Navigator）亲授的直播英文读写课程，覆盖 5 岁到高中。都学启蒙（K–2）：由幼儿教育导师授课的语音启蒙阅读；ELA 课程（3 年级及以上）：完整的 Read → Think → Speak → Write 循环，每个 16 周周期 Lexile 平均提升一个年级。以 MCT 语言艺术框架为根基。双语深度从认知严谨中自然涌现。',
  },
  ...
  photoIntro: {
    ...
    body0:
      '学校知道孩子在几年级。那不等于知道孩子真正站在哪里。很多孩子都有一个成绩单上看不见的阅读差距——能认出的词，和能真正思考的内容，中间隔着一段距离。这段距离，大多数课程测不出来。',
    body1:
...
export const program = {
  ...
  architecture: {
    eyebrow: 'The LCS 体系架构',
    h2:      '成果如何累积：The Loop（每节课） → The LCS（每个周期） → 级别（跨多个周期）。',
    h2zh:    null,
    body:
      'LCS 教学理念——文学精读 · 系统写作 · 表达——是课程的架构。' +
      '在 LCS 之内，每节课运行 The Loop，由您孩子的导师引导。' +
      '每个16周周期内，每节课累积为 LCS 三个维度的进步。' +
      '跨越多个周期，学生逐步进阶九个课程级别——Starter、Intermediate 以及 Levels 1–7。' +
...
export const about = {
  meta: {
    ...
    description:
      'DODO Learning是面向全球家庭的、由导师（Navigator）带领的直播英语读写项目——基于完整的阅读 → 思考 → 表达 → 写作循环进行训练，通过Lexile水平和6+1特质写作框架进行衡量。我们培养达到精通水平的英语思维者。双语深度从这种严谨性中自然涌现。',
  },
  hero: {
    chip:          '我们的故事',
    ...
    sub:
      '专为全球家庭打造——这些家庭的孩子将在英语主导的学校、大学和职场中成为引领者。他们中的许多孩子把英语当作一门学科来学——能通过考试，听起来流利。但让他们论证一个观点、阅读密集的分析性散文、或写出有原创性的东西——语言就达到了极限。DODO 的创始人 Janet 看到了这个差距，并建立了 DODO 来弥合它。这项工作早在 5 岁开始，一直延续到高中：都学启蒙的语音、流利度与阅读的乐趣；ELA 课程里完整的 Read → Think → Speak → Write 循环。目标是在认知层面达到英语精通。双语深度是达到这一目标后自然产生的结果。',
  ...
  beliefs: {
    ...
    bodies: [
      ...
      '英语中的认知深度，会保护并强化两种语言的思考能力。孩子用英语推理越精确，他们在使用的每一种语言中的思考就越精妙。一个能读懂缜密分析性文章、能用正式英语论证立场、能精确写作，同时又用母语思考的孩子，拥有一种机器无法取代的头脑。双语能力正是这种严谨的自然印证——而不是一个需要单独打理的目标。',
  ...
  loop: {
    ...
    descs: [
      ...
      '阐明并支持一个立场。代入角色的视角。精确说明证据在文本中的哪个位置。表达，是让思考精确到足以写作的方式。',
    ...
    items: [
      {
        ...
        desc:
          '您明白，英语精通和母语深度并非相互竞争的目标。一个被训练用一门语言精确思考的孩子，会将这种精确性带入两门语言。您希望标准定得高——并且可以被衡量。',
      },
      {
...
export const compare = {
    meta: {
      ...
      description: 'DODO Learning是面向全球家庭设计的认知发展项目——不是补习中心，不是ESL项目，不是备考辅导。以下是它在结构上的根本区别，以及这种区别如何在16周内产生累积效应。',
    },
    s1: {
