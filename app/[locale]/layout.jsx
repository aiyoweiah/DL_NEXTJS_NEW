...
import cjkPreload from '@/lib/cjk-preload.json'   // GENERATED — npm run fonts:cjk
  ...
  return (
    <>
      ...
      */}
      ...
      {/*
        Preload the CJK chunks a Chinese page is certain to need, on zh
        routes only. EN routes render at most the language switcher, which
        the browser fetches from the 2 KB `chrome` chunk on demand.

        The hrefs come from lib/cjk-preload.json, which is GENERATED by
        scripts/build-cjk-subset.mjs alongside the chunks themselves.
        This used to be a hand-written href pointing at
        `/_next/static/media/noto-sans-sc.woff2` — a filename that never
        existed — so it 404'd on every Chinese page. A dead preload fails
        silently, which is why it survived. Generating the list means the
        href cannot drift from the file on disk again.

        Next.js App Router hoists <link> elements from layouts into <head>.
      */}
      {locale === 'zh' &&
        cjkPreload.chunks.map((chunk) => (
          <link
            key={chunk.file}
            rel="preload"
            as="font"
