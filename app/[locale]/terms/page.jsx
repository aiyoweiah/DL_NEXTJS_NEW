    ...
    sections: [
      ...
      },
      {
        ...
        p: [
          ...
          '受保护内容 — 包括「有声书」内容库、合作伙伴材料及任何学员专属资源 — 仅授权在校学员及其直系家庭于入学期间使用。共享访问凭据或转载受保护内容均不允许。',
  ...
  return (
    <article className="container-section py-16 md:py-20">
      <header className="mb-10 max-w-2xl">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-[color:var(--text-heading)]">
          {c.title}
        </h1>
      ...
      <div className="max-w-2xl space-y-10">
        {c.sections.map((s, i) => (
          <section key={i}>
            <h2 className="text-xl font-semibold mb-3 text-[color:var(--text-heading)]">
              {s.h}
            </h2>
            <div className="space-y-3">
              {s.p.map((para, j) => (
                <p key={j} className="text-base leading-relaxed text-[color:var(--text-body)]">
                  {para}
                </p>
              ))}
            </div>
