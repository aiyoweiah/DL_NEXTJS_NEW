> **ARCHIVED 2026-09-03, verbatim.** This is the interface guide as it stood at v6.44 (D90),
> before the 2026-09-03 restructure split the normative rulebook from its decision history.
> The live guide is `.interface-design/system.md` (v7.0+). Status authority: `docs/decision-index.md`.
> Nothing below is guidance; it is the historical record, including the D33-D90 narratives.
...
| D83 | **The decorative fifteen — fixed without changing a pixel** | Item 3, group 1. Four situations carried no information and were failing AA purely as ornament: `/consult`'s 32px ghost step numerals (**1.11:1**, the worst on the site), `/compare`'s 11px column ordinals, the home page's `·` phrase separator, and `/results`' trait arrows. The first three take `aria-hidden="true"` — WCAG exempts decorative content, so the failure goes away with identical pixels. ⚠️ **The arrows were misclassified in the review and the gate caught it**: they sit *between two numbers* (`2.1 → 4.2`), so hiding them would have left assistive tech reading "2.1 4.2" and losing the relationship. They got the glyph hidden plus an `sr-only` "improving to" instead — visually byte-identical, and the accessible reading improves from `2.1 4.2 / 6` to `2.1 improving to 4.2 / 6`. **105 → 90 failing nodes, verified on the rendered DOM.** No token, colour or size changed | ✅ v6.37 |

| D84 | **Supporting text off alpha — 2a applied, and a conversion CTA found at 1.29:1** | Item 3, group 2. **102 `color:` declarations** migrated: lavender supporting text → `--text-muted-dark` (2a — it failed on every dark ground below α .8, so alpha was never the right instrument), platinum text → `--platinum-60` (the D82 floor; nothing below .60 is a step), plus two light-ground cases → `--text-accent` and `--text-muted`. Every replacement moves contrast **up**; no element can regress. ⚠️ **Found while gating, pre-existing and unrelated to this change: `/compare` renders a `btn-do-primary` CTA — "Book Your Consultation" — with a near-black `--text-body-dark` label on a hand-painted `#212830` ground and no `.on-dark` hook. 1.29:1.** That is precisely the trap `check-gilt-escrow`'s own fix message warns about (D53/D65). Not fixed here; it is a conversion control and deserves its own change. ⚠️ **Also: the audit instrument is not trustworthy for a final count.** Walking ancestors for an opaque `backgroundColor` cannot see a ground painted by an overlay or pseudo-element, so it reports false failures on `/program` and `/demos` and gave a different answer in a detached DOM than on the live page. The "105" and "24" figures are directionally right and not certifiable | ✅ v6.38 |

| D85 | **The `on-dark` trap is now guarded — and the pixel auditor is deliberately NOT built** | `/compare` defined a **local `Section`** duplicating `SectionWrapper`; it painted `#212830` inline and never added the hook, so its s9 `.btn-do-primary` shipped a `--text-body-dark` label at **1.29:1** on a conversion CTA, both locales. The system's own fix message had warned of exactly this since **D53**. Fixed by deriving `on-dark` from `bg` inside that component — **1.29:1 → 13.06:1** — and guarded by new `check-on-dark.mjs` on `postbuild`, written *before* the fix and confirmed to fail on the live bug first. **The design ruling matters as much as the fix: contrast is two problems, not one.** *Structural* failures (a control on a ground missing its hook) are decidable from the DOM — a class check plus an ancestor check, ~40 lines, zero false positives, and they are the ones that ship broken conversion controls. *Value* failures (a colour too faint) need composited pixels, hence a browser in the build — which the cohesion proposal forbids ("the build must not acquire an install step"). **So the pixel auditor is not being built.** Value-level contrast is a design review, recorded as such rather than covered by a guard that does not exist. 14 guard passes now run on a build | ✅ v6.39 |

| D86 | **537 colour literals tokenised — and two guards that would have gone blind** | Wave 1 step 1 (tokenise; extraction is step 2 and does not move the ratchet). **537 exact-match hex literals → semantic tokens** across `app/` + `components/`, mapped by role: as text `#0E0E12` → `--text-body-dark`, as surface → `--color-void-black`, and so on for ten colours. All 14 verified in the browser to resolve to exactly the literal they replaced — **zero visual change**. One genuine bonus: `--text-body` is redefined under `@media (prefers-contrast: more)`, so **41 elements now honour the high-contrast preference** that a hardcoded hex could never respond to. ⚠️ **Two guards matched on the old spelling and had to be taught the new one.** `check-surfaces` reported **24 panels → 5** — not migration, blindness: its `PANEL` regex listed hex only, so 19 panels vanished the moment their background became `var(--color-midnight)`. Caught only because the drop was implausibly large; a fall from 24 to 22 would have been banked as progress. `check-on-dark` had the same coupling and was fixed *pre-emptively* before the tokenisation ran. **A ratchet that goes green on a rename is worse than no ratchet**, and both files now say so at the regex | ✅ v6.40 |

| D87 | **Match on structure, never on values — and refuse to bank an implausible drop** | The permanent answer to D86's blindness, and it was already demonstrated in-repo: `check-surfaces`' **build** pass asks whether the *properties* are set and held steady at 737 through the rename, while its **source** pass enumerated background colours and collapsed 24 → 5. **Two fixes.** (1) The source detector no longer lists values at all: a panel is an inline background plus an inline **full** border, brace-matched. That also revealed the list was never right — the true count is **41, not 24**, so 17 panels had been invisible since D60 and the ratchet was guarding a number too low for months. Re-baselined at 41. (2) New `implausibleDrop()` in **both** ratchets: a fall of >30% and ≥8 items refuses `--update` without `--force`, because **the moment a ratchet lies is the moment you bank the lie** — a broken detector and a successful migration look identical, and `--update` is what makes the smaller number the new truth. Verified: a fabricated 99 → 41 is refused with exit 1; `--force` still works and now means "I checked". The build pass needs neither fix, which is the argument for structural matching in one line | ✅ v6.41 |

| D88 | **Extraction is not a mechanical transform — inline styles here are load-bearing for specificity** | Wave 1 step 2, **attempted and reverted**. 187 single-property colour blocks (`{{ color: 'var(--text-muted-dark)' }}` ×47 across 10 files, and 21 similar) were extracted to Tailwind arbitrary utilities. Every promised metric moved: source **933 → 746**, build **8,709 → 4,518**, inline-style payload **731 KB → 585 KB**. **And 8 elements on `/results` alone rendered the wrong colour.** Cause: `.section-dark p`, `.on-dark h2` and friends colour their descendants, and a utility class loses to them where an inline style could never lose. **The section rules were written assuming inline wins** — so inline colour on this site is not laziness, it is specificity, and extracting it silently hands those elements to the cascade. Reverted whole. ⚠️ **Consequence for Wave 1: the ratchet cannot be driven down by extraction until the `.section-*` descendant colour rules are refactored.** Three routes — `!important` utilities (preserves behaviour, encodes the bug), refactor the section rules (the real fix, a design-system job), or accept a lower ceiling: after D86 the values are tokens and `check-tokens` already covers them, 55% of blocks are one-offs, and payload is the only remaining prize. **D87's banking gate fired on this run** — it refused the 48% build drop and asked for `--force`. First real use, and it was right to ask | ✅ v6.42 |

| D89 | **Section colour rules become defaults, not overrides** | Finishes a fix the system wrote for itself in **v6.5** and applied to links only. `.section-dark p` / `.on-dark h2` and their siblings are (0,1,1), so they outrank any utility or component class — **overrides where they were only ever meant to be defaults**. Invisible while every colour was inline (inline beats any rule); the moment D88 moved 187 colours into classes, they started winning and eight elements rendered wrong. v6.5's own comment states the rule — *"a section-scoped descendant selector must never set a component's colour"* — and fixed it with `:not(.btn)` on links. Headings and paragraphs never got it. **22 selector lines across 2 blocks wrapped in `:where()`**, dropping them to (0,0,1): identical for anything setting no colour, overridable by anything that does. The `@media print` block is deliberately untouched. **Measured, not eyeballed: 315 heading/paragraph elements across 8 dark pages, 309 unchanged, 6 changed** — all on `/methodology`, all restoring an author intent the rule had been suppressing (a Lexile stat block now reads accent-number + muted-caption instead of uniform platinum). All six verified AA with proper alpha compositing through a 6% tint: **9.28:1** at 36px, **6.93:1** at 15px. Unblocks Wave 1 step 2 | ✅ v6.43 |

| D90 | **Wave 1 step 2 — the extraction that D88 reverted, now safe** | Same 187 single-property colour blocks, same script, redone on top of D89. **Ratchet: source 933 → 746, build 8,709 → 4,518.** Inline-style payload **731 KB → 585 KB**. Verified on three dark pages with real navigation (not a detached DOM): **zero elements overridden**, where the same probe found 8 on `/results` before D89. Banked with `--force` — D87's gate refused the 48% build drop and was right to, so the drop was verified first and the flag then means "I checked". ⚠️ **A fourth instrument lesson.** My before/after capture rendered pages into an iframe via `document.write` and counted 22 heading/paragraph elements on `/results` where the built HTML has 19 — a consistent **−3 on seven of eight pages**, from hydration timing, not from the change. Caught by A/B-ing the actual build (`git stash`, rebuild, parse `out/` with `html-parse`): **19 before, 19 after, nothing lost.** An iframe is not the page; parse the built output or navigate for real | ✅ v6.44 |
