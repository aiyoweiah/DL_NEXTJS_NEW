...
| 20 | **Little DODO — K–2 ELA sub-program (2026-06-02)** — new program briefed (D32). Shares operations (tuition, environment, frequency, Navigators); differs on marketing emphasis + audience (K–2 pre-elementary, high-frequency low-pressure reading + comprehension). Page build deferred (Task 3) — must **strictly follow `.interface-design/system.md`** with K–2-appropriate warmth. Positioning + page plan in `docs/little-dodo-plan.md`. **OPEN sub-decisions:** (a) URL/route (`/little-dodo` vs `/program/little-dodo`); (b) how it relates to the "ELA Program" nav item (keep flat / branch the program page / future dropdown — current rule is no dropdown); (c) ZH program name; (d) age-band copy (K–2 vs ages). | ⬜ **Pending — plan recorded, build on greenlight** |
