...
import { DM_Sans, Noto_Sans_SC, Literata, Noto_Serif_SC } from 'next/font/google'
...
// ── Literata — Latin display face (D51, 2026-08-27) ───────────
// DISPLAY ONLY: h1/h2 on strategic surfaces. Never body, UI, labels,
// or buttons — those stay on DM Sans. Never below 24px, where it
// fights DM Sans instead of complementing it.
// See .interface-design/system.md § "Typeface pairing".
//
// Drawn by TypeTogether for Google Play Books, for long-form reading.
// A literacy brand setting its headlines in a face built for readers
// is an argument, not a decoration. Deliberately NOT Playfair Display
// or Fraunces — those are the AI-default display serifs and read as
// generic (the ui-ux-pro-max font DB recommends Playfair + Inter for
// this brief; it was rejected for that reason).
//
// Weights: 400 (light display), 500 (standard), 600 (emphasis).
// preload: false — the display face is applied per-surface during
// rollout, not sitewide. Flip to true once it lands above the fold
...
// ── Noto Serif SC (思源宋体) — CJK display face (D51) ──────────
// The required counterpart to Literata. A Latin serif with no CJK
// pair breaks every /zh page — the ZH side is where a type change
// actually breaks, so it ships in the same commit, not later.
//
// Same superfamily as Noto Sans SC, so body and display metrics stay
// coherent on Chinese surfaces.
//
// Weights: 500 (standard display), 600 (emphasis). 400 is excluded —
// at display sizes it reads too light beside Literata 500.
// adjustFontFallback: false for the same reason as Noto Sans SC —
// fallback metrics differ too much for size-adjust to help.
export const fontDisplayCJK = Noto_Serif_SC({
  subsets: ['chinese-simplified', 'latin'],
  weight: ['500', '600'],
  style: ['normal'],
  ...
  display: 'swap',
  preload: false,
  fallback: ['Songti SC', 'SimSun', 'Noto Serif CJK SC', 'serif'],
  adjustFontFallback: false,
})
