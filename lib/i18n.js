...
// ── Content module map ────────────────────────────────────────
// Explicit static imports — required for bundlers (Turbopack, webpack) to
// statically analyse the dependency graph. A template literal like
// import(`@/content/${locale}.js`) is not statically analysable and will
// cause build failures in StackBlitz and some CI environments.
// Adding a new locale means adding one line here.
const CONTENT_MODULES = {
  en: () => import('@/content/en.js'),
  zh: () => import('@/content/zh.js'),
}

/**
 * Loads the named page export from content/{locale}.js.
 *
...
export async function getContent(locale, page) {
  ...
  try {
    const loader = CONTENT_MODULES[resolvedLocale]
    const mod    = await loader()

    // Module loaded but the page export is missing or empty stub — fall back to EN
    if (!mod[page] || (typeof mod[page] === 'object' && Object.keys(mod[page]).length === 0)) {
      throw new Error(`export "${page}" not found or empty in content/${resolvedLocale}.js`)
    }
