...
// Default OG image per locale — D109 (2026-09-09): candidate C, light ground, centred so the
// WeChat / Moments 1:1 centre-crop keeps the whole composition. EN card is og-default.png (kept
// under the old name so cached external references update); ZH card is og-zh.png. 1200 × 630.
// Copy on the cards is locked strings only: wordmark, first-mention pair, D36 tagline, domain.
const OG_IMAGE_DEFAULT = {
  en: { url: `${SITE_URL}/og-default.png`, width: 1200, height: 630, alt: 'DODO Learning · 都学书院 — Think once, in two languages.' },
  zh: { url: `${SITE_URL}/og-zh.png`,      width: 1200, height: 630, alt: 'DODO Learning · 都学书院 — 一次思考，两种语言。' },
}
...
export function buildMetadata({
  ...
  const fullTitle =
    title && title.includes(SITE_NAME)
      ...
      : title
        ? `${title} ${TITLE_SUFFIX}`
        : `${SITE_NAME} — Think once, in two languages.`

  const image = ogImage ?? OG_IMAGE_DEFAULT[locale] ?? OG_IMAGE_DEFAULT.en
  ...
  return {
    ...
    // ── App icons (all served from public/) ──────────────────
    // favicon.ico        — legacy browsers + bare /favicon.ico request
    // favicon.svg        — modern Chrome/Firefox/Edge; crisp at any size
    // favicon-96x96.png  — pinned tabs, some Android browsers
    // apple-touch-icon   — iOS home screen
    //
    // NOTE: app/favicon.ico must NOT exist alongside this explicit icons
    // block — the file convention and explicit declaration conflict under
    // output: 'export', causing the favicon to silently 404.
    icons: {
      icon: [
...
export function buildCityMetadata(citySlug, locale = 'en', cityName) {
  ...
  // Localised pairs (D40: demographic explicit on local surfaces). The program name is the
  // ELA Program — "16-Week Program" is retired (§09; canon guard).
  const copy = locale === 'zh'
    ? {
        title:       `DODO Learning · ${name}`,
        description: `DODO Learning 服务${name}的华语家庭。真人在线、导师（Navigator）亲授的英文语言艺术课程，` +
                     `覆盖 3 年级到高中，另有面向 5–8 岁的都学启蒙。以 Lexile（蓝思）衡量进步。ELA 课程——阅读、思考、表达、写作。`,
      }
    : {
        title:       `DODO Learning in ${name}`,
        description: `DODO Learning serves Chinese-speaking families in ${name}. Live, online, Navigator-led English language arts ` +
                     `from Grade 3 through high school, with Little DODO for ages 5–8. Progress measured in Lexile. ` +
                     `The ELA Program — read, think, speak, write.`,
      }
  return buildMetadata({ locale, title: copy.title, description: copy.description, path: `/cities/${citySlug}` })
}
