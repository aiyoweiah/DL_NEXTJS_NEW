...
// ── Content version — bump on every content cascade ──────────
// Machine-readable freshness. Every AEO/GEO instrument run on 2026-09-09 scored the site
// down for carrying no dateModified anywhere; this constant feeds the page-level nodes.
// Bump it in the same commit as any apply-gated copy cascade (D99-style).
export const CONTENT_MODIFIED = '2026-09-09'

// ── Founder reference node ────────────────────────────────────
// Person authorship is the E-E-A-T signal the instruments look for (best-aeo Rule 41,
// audit-website-aeo "0% author attribution"). The full Person node lives on /about
// (personSchema); every other page carries this minimal node with the same @id so the
// graph merges. Name rule D35: first name only.
const founderRef = () => ({
  '@type':   'Person',
  '@id':     `${SITE_URL}/#founder`,
  name:      'Janet',
...
const langOf = (locale) => (locale === 'zh' ? 'zh-Hans' : 'en')

// ── Locale copy for the machine layer (existing meta strings, no new copy) ──
const COURSE_NAME = {
  en: 'English Language Arts Program — DODO Learning',
  zh: 'ELA 课程 — DODO Learning',
}
const COURSE_DESCRIPTION_ZH =
  '真人、在线、导师亲授的英文语言艺术课程。语言循环体系与 MCT 课程为架构，每节课运行 The Loop — ' +
  '阅读 → 思考 → 表达 → 写作。入学、中期、结业三次Lexile评估。每个16周周期Lexile阅读水平提升约一个年级。'
const LITTLE_DODO_NAME = { en: 'Little DODO', zh: '都学启蒙（Little DODO）' }
const LITTLE_DODO_DESCRIPTION_ZH =
  '都学启蒙是面向 5–8 岁（K–2）孩子的高频低压英文阅读启蒙课程。一位专属导师（Navigator）直播一对一，' +
  '培养阅读理解力与稳定的理解习惯——通往都学 ELA 课程的温和起点。'
...
export function websiteSchema() {
  return {
    ...
    url:           SITE_URL,
    name:          'DODO Learning',
    alternateName: ['DODO', '都学书院', 'DODO Learning Canada', 'DODO Learning Program'],
...
export function educationOrgSchema() {
  return {
    ...
    alternateName: ['DODO', '都学书院', 'DODO Learning Canada', 'DODO Learning Program'],
    url:          SITE_URL,
    // D109 (2026-09-09): the square "DO" mark, not the social card — knowledge panels and
    // citation cards want a 1:1 logo; the card is the og:image, set in lib/metadata.js.
    logo: {
      '@type':  'ImageObject',
    ...
    audience: {
      ...
      audienceType:    'Capable K–12 students seeking English at mastery level',
    },
    ...
    // sameAs — D105 (2026-09-09): the YouTube channel is confirmed as DODO's.
    // Xiaohongshu / WeChat OA URLs pending (workflow Open Decision #9).
    sameAs: ['https://www.youtube.com/@DODO-Learning'],
  }
}
...
export function faqSchema(items = [], locale = 'en') {
  return {
    ...
    author:     { '@type': 'EducationalOrganization', '@id': `${SITE_URL}/#organization`, name: 'DODO Learning' },
 ...
 */
export function courseSchema(locale = 'en') {
  return {
    '@context': 'https://schema.org',
    '@type':    'Course',
    '@id':      `${SITE_URL}/${locale}/program/#course`,

    name:        COURSE_NAME[locale] ?? COURSE_NAME.en,
    description: locale === 'zh' ? COURSE_DESCRIPTION_ZH :
      'The DODO English Language Arts Program (ELA Program) is a live, Navigator-led English literacy program ' +
      'for capable students from Grade 3 through high school seeking English at mastery level. Each cycle runs ' +
    ...
    audience: {
      '@type':         'EducationalAudience',
      educationalRole: 'student',
      audienceType:    'Capable students from Grade 3 through high school seeking English at mastery level',
    },
    ...
    educationalLevel: 'Grade 3 through high school (seven-level ELA ladder)',
...
export function littleDodoCourseSchema(locale = 'en') {
  return {
    '@context': 'https://schema.org',
    '@type':    'Course',
    '@id':      `${SITE_URL}/${locale}/little-dodo/#course`,

    name:        LITTLE_DODO_NAME[locale] ?? LITTLE_DODO_NAME.en,
    description: locale === 'zh' ? LITTLE_DODO_DESCRIPTION_ZH :
    ...
    url:        `${SITE_URL}/${locale}/little-dodo/`,
    inLanguage: langOf(locale),
    author:     founderRef(),
    dateModified: CONTENT_MODIFIED,
 ...
 */
export function credentialsSchema(locale = 'en') {
  return {
    '@context': 'https://schema.org',
    '@type':    'CollectionPage',
    '@id':      `${SITE_URL}/${locale}/credentials/#page`,
    author:     founderRef(),
    dateModified: CONTENT_MODIFIED,
    ...
    url:         `${SITE_URL}/${locale}/credentials/`,
    inLanguage:  langOf(locale),
...
// EducationalOrganization scoped by areaServed — D104 (2026-09-09): no LocalBusiness type, no PostalAddress; the program has no premises in any city.
 ...
 */
export function citySchema({ name, slug, region, country, locale = 'en' }) {
  return {
    '@context': 'https://schema.org',
    '@type':    'EducationalOrganization',
    '@id':      `${SITE_URL}/${locale}/cities/${slug}/#organization`,

    name:        `DODO Learning — ${name}`,
    url:         `${SITE_URL}/${locale}/cities/${slug}/`,
    description:
      `DODO Learning serves Chinese-speaking families in ${name}, ${region}. ` +
      `Live, Navigator-led English language arts from Grade 3 through high school, with Little DODO for ages 5–8. ` +
...
export function articleSchema(post) {
  ...
  return {
    ...
    image: post.ogImage
      ...
      : { '@type': 'ImageObject', url: `${SITE_URL}/${locale === 'zh' ? 'og-zh.png' : 'og-default.png'}` },
